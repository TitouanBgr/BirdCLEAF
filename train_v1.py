...
def train_model(train_data, val_data, model, epochs=30):  # Reduce epochs to 30
...
X_train = np.random.rand(100, 128, 384, 1)
y_train = np.random.randint(0, CFG.num_classes, 100)
X_val = np.random.rand(20, 128, 384, 1)
y_val = np.random.randint(0, CFG.num_classes, 20) 
...
input_shape = (CFG.img_size[0], CFG.img_size[1], 1)
model = create_model(input_shape, CFG.num_classes)
train_model(train_data, val_data, model, epochs=30)  # Reduce epochs to 30
